...
export class VisualStudioExporter extends Exporter {
	...
	async additionalFiles(Icon: any, from: string, to: string, project: Project, platform: string) {
	...
	itemDefinition(config: string, system: string, includes: string, debugDefines: string, releaseDefines: string, indent: number,
		...
		if (Options.precompiledHeaders) this.p('<PrecompiledHeader>Use</PrecompiledHeader>', indent + 2);
		...
		if (project.isCmd()) this.p('<SubSystem>Console</SubSystem>', indent + 2);
		else this.p('<SubSystem>Windows</SubSystem>', indent + 2);
		if (project.livePP && config !== 'Release') {
